...
const SlideCardAction = ({
  ...
  return (
    ...
    >
      <div className=" hidden md:flex items-center justify-center relative h-[2.15rem] w-[1.75rem] bg-stone-300 z-30 rounded-sm shrink-0  overflow-hidden lg:w-[2.5rem] lg:h-[3.20rem] xl:h-[4.5rem] xl:w-[3.5rem]">
        <Image src={image} className=" w-full h-full object-cover" />
      </div>
      <p className=" hidden md:block truncate text-xs text-white font-medium z-30 lg:text-sm xl:text-lg">
        {title}
      </p>
      <div
