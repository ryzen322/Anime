...
const SlideCardAction = ({
  ...
  return (
    ...
    >
      <div className=" hidden md:flex items-center justify-center relative h-[2.15rem] w-[1.75rem] bg-stone-300  rounded-sm shrink-0  overflow-hidden lg:w-[2.5rem] lg:h-[3.20rem] xl:h-[4.5rem] xl:w-[3.5rem]">
        <Image src={image} className=" w-full h-full object-cover" />
      </div>
      <p className=" hidden md:block truncate text-xs text-white font-medium  lg:text-sm xl:text-lg">
        {title}
      </p>
      <div
        className={` hidden md:block h-full ${
          active ? "w-full" : `w-[0%]`
        } absolute top-0 left-0 bg-[#404044] -z-10 transition-all duration-[4000ms] ease-linear`}
      ></div>
    </li>
  );
};
